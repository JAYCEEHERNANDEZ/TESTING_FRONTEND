...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [consRes, userRes] = await Promise.all([fetchConsumptions(), fetchUsers()]);
  ...
  const filteredConsumptionsForPDF = consumptions.filter(c => {
    const date = new Date(c.billing_date || c.created_at);
    return (!pdfFilterYear || date.getFullYear() === Number(pdfFilterYear)) &&
           (!pdfFilterMonth || date.getMonth() + 1 === Number(pdfFilterMonth));
  });

  const filteredUsersForPDF = users.filter(u => {
    const created = new Date(u.created_at);
    return (!pdfFilterYear || created.getFullYear() === Number(pdfFilterYear)) &&
  ...
  if (pdfFilterYear && pdfFilterMonth) {
    // Monthly record
    const monthName = new Date(0, pdfFilterMonth - 1).toLocaleString("default", { month: "long" });
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ["Overall Bill", ` ${filteredConsumptionsForPDF.reduce((sum, c) => sum + Number(c.total_bill || 0), 0).toLocaleString()}`],
      ["Total Balance", ` ${filteredConsumptionsForPDF.reduce((sum, c) => sum + Number(c.remaining_balance || 0), 0).toLocaleString()}`],
      ["Total Income", ` ${filteredConsumptionsForPDF.reduce((sum, c) => sum + Number(c.payment_total || 0), 0).toLocaleString()}`],
      ["Cubic Meters Used", filteredConsumptionsForPDF.reduce((sum, c) => sum + Number(c.cubic_used || 0), 0)],
    ],
    theme: "grid",
    ...
    <SideBarHeader>
      <div className="flex justify-between mb-6">
        ...
        <div className="flex gap-2">
          <select value={pdfFilterMonth} onChange={e => setPdfFilterMonth(e.target.value)} className="p-2 rounded shadow-inner">
            <option value="">All Months</option>
            {Array.from({ length: 12 }, (_, i) => (
              <option key={i + 1} value={i + 1}>
                {new Date(0, i).toLocaleString("default", { month: "long" })}
          ...
          <select value={pdfFilterYear} onChange={e => setPdfFilterYear(e.target.value)} className="p-2 rounded shadow-inner">
            <option value="">All Years</option>
            {years.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <button onClick={generatePDF} className="bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700">Export PDF</button>
        </div>
      </div>
